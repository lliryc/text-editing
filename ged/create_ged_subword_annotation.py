...
def merge_annotations(areta_annotation_word_level_df, edits_annotation_subword_level_df, output_label_column, output_areta_annotation_subword_level_file, pnx=False):
  ...
  while subword_pos < len(edits_annotation_subword_level_df) and word_pos < len(areta_annotation_word_level_df):
     ...
     original_edit = edits_annotation_subword_level_df.at[subword_pos, 'Edit']
...
def merge_annotations_with_edits(areta_annotation_word_level_df, edits_annotation_subword_level_df, output_label_column, output_areta_annotation_subword_level_file, pnx=False):
  # create datasource for the new data frame from tuples list
  subword_error_list = []
  ...
  while subword_pos < len(edits_annotation_subword_level_df) and word_pos < len(areta_annotation_word_level_df):
     ...
     if current_subword == current_word[:len(current_subword)]:
       subword_error_list.append((original_edit, current_error_label))
       subword_pos += 1
       current_word = current_word[len(current_subword):]
       if current_word == '':
         word_pos += 1
       ...
       # Try to advance both positions to find the next match
        ...
        subword_error_list.append((original_edit, 'UC'))
        subword_pos += 1

  with open(output_areta_annotation_subword_level_file, 'w') as f:
    for edit, error_label in subword_error_list:
      if edit is None:
        f.write('\n')
      else:
        f.write(f"{edit}\t{error_label}\n")

if __name__ == "__main__":
  parser = argparse.ArgumentParser()
  parser.add_argument("--input_areta_annotation_word_level", default="/Users/kirill.chirkunov/CursorProjects/text-editing/data/msa-ged/raw/train/qalb14-train-word-level-areta.tsv", type=str, required=True)
  parser.add_argument("--input_edits_annotation_subword_level", default="/Users/kirill.chirkunov/CursorProjects/text-editing/data/msa-gec/modeling/qalb14/pnx_sep/qalb14-nopnx/compressed/subword-level/train.txt", type=str, required=True)
  parser.add_argument("--output_areta_annotation_subword_level", default="/Users/kirill.chirkunov/CursorProjects/text-editing/data/msa-ged/modeling-13/pnx_sep/nopnx/train/qalb14-train-subword-level-edits-areta-13.txt", type=str, required=True)
  ...
  edits_annotation_subword_level_df = pd.read_csv(args.input_edits_annotation_subword_level, sep="\t", names=['Input', 'Edit'], header=None, quoting=3, skip_blank_lines=False)

  merge_annotations_with_edits(areta_annotation_word_level_df, edits_annotation_subword_level_df, args.output_label_column, args.output_areta_annotation_subword_level)
