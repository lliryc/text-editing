...
def ensemble_rewrite(models_edits, voting_threshold=2):
    ...
    examples = [
        {(edit.subword, idx): Counter() for idx, edit in enumerate(subword_edits['subword-edits-append'])}
    ...
    for i, edits_group in enumerate(zip(*models_edits)):
        for model_edits in edits_group:
            for idx, edit in enumerate(model_edits['subword-edits-append']):
    ...
    for i, example in enumerate(examples):
        ...
        for j, subword in enumerate(example):
            ...
            if edit:
                ...
                if edit == 'D':
                    edit = 'D*'
                _example.append(SubwordEdit(subword[0], edit))
            else:
                # import pdb; pdb.set_trace()
...
if __name__ == '__main__':
    ...
    input_data = read_data(args.input_data)
    ...
    for model_output in models_outputs:
        ...
        data = [{'raw': input_data[i]['raw'], 'cor': model_output[i]} for i in range(len(input_data))]
        edits_data = create_dataset_edits_parallel(data, tokenizer, direction='raw-cor', num_workers=100)
        models_edits.append(edits_data)

    rewritten_examples = ensemble_rewrite(models_edits, voting_threshold=2)

    rewritten_examples = pnx_tokenize(rewritten_examples)
    rewritten_examples_nopnx = remove_pnx(rewritten_examples)


    with open('/scratch/ba63/arabic-text-editing/ensemble/dev.ensemble.txt', mode='w') as f:
        for example in rewritten_examples:
            f.write(example)
            f.write('\n')


    with open('/scratch/ba63/arabic-text-editing/ensemble/dev.ensemble.nopnx.txt', mode='w') as f:
