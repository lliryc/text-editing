...
from utils import get_labels, process, TokenClassificationDataset, DataCollatorForTokenClassification, read_examples_from_file, load_ged_labels
...
class ModelArguments:
    ...
    keep_confidence_bias: Optional[float] = field(
        default=0.0, metadata={"help": "Keep confidence bias"}
    )
...
def _align_predictions_topk(predictions, label_ids, label_map, k=10, keep_confidence_bias=0):
    """Aligns the predictions of the model with the inputs and it takes
    care of getting rid of the padding token.
    Args:
        predictions (:obj:`np.ndarray`): The predictions of the model
