...
import json
import editdistance
from .ced_alignment import align_words, align_chars
...
def perfect_align(src, tgt, is_char_align=False):
    ...
    while i < len(src):
        ...
        if (src[i] == 'NIL' and tgt[i] != 'NIL'): # insert or a potential split
            ...
            prepend = is_split_merge(src, tgt, i, 'prepend', is_char_align=is_char_align,
                                     src_first=True)
            append = is_split_merge(src, tgt, i, 'append', is_char_align=is_char_align,
                                    src_first=True)
            best_edit = get_best_edit(best_edit, prepend, append)

        elif (src[i] != 'NIL' and tgt[i] == 'NIL'): # delete or a potential merge

            prepend = is_split_merge(tgt, src, i, 'prepend', is_char_align=is_char_align,
                                     src_first=False)
            append = is_split_merge(tgt, src, i, 'append', is_char_align=is_char_align,
                                    src_first=False)
            best_edit = get_best_edit(best_edit, prepend, append)
...
def is_split_merge(src, tgt, i, mode='prepend', src_first=False, is_char_align=False):
    ...
    if mode == 'prepend':
        ...
        if j >= 0:
            ...
            if is_char_align: # we dont add spaces for characters when we merge
...
def bug_fix(align, seq_bug, is_char_align=False):
    ...
    for bug in seq_bug:
        ...
        # fix their alignment greedily in case of words (i.e., merges and splits)
        _p_src, _p_tgt = perfect_align(p_src, p_tgt, is_char_align)
        src += _p_src
        tgt +=  _p_tgt
...
def post_process_alignment(alignment, is_char_align=False):
    ...
    for i, align in enumerate(alignment):
        ...
        if len(seq_bug) == 0:
            ...
            src, tgt = bug_fix(align, seq_bug, is_char_align=is_char_align)
            clean_alignment.append({'src': src, 'tgt': tgt})
...
def verify(src_sents, tgt_sents, alignment):
    ...
    for src, tgt, align in zip(src_sents, tgt_sents, alignment):
        ...
        src_ = ' '.join([x for x in align['src']])
        tgt_ = ' '.join([x for x in align['tgt']])

        assert re.sub(' +', ' ', src).strip() == re.sub(' +', ' ', src_).strip()
        assert re.sub(' +', ' ', tgt).strip() == re.sub(' +', ' ', tgt_).strip()


def word_level_alignment(src_sent, tgt_sent):
    """Generating word level alignment for a pair of src and tgt
    sentences.

    Args:
        src_sent (str): src sentence
        tgt_sent (str): tgt sentence

    Returns:
        word_alignment (dict): pair of word-level aligned sentences
    """
    # create basic alignments first
    basic_alignment =  align_words(src_sent, tgt_sent)

    # clean up the basic alignment
    clean_alignment = post_process_alignment([basic_alignment], is_char_align=False)

    # verify that the alignment is correct
    verify([src_sent], [tgt_sent], clean_alignment)

    word_alignment = {'src': [], 'tgt': []}

    assert len(clean_alignment) == 1

    aligned_pair = clean_alignment[0]
    aligned_src, aligned_tgt = aligned_pair['src'], aligned_pair['tgt']

    for _src, _tgt in zip(aligned_src, aligned_tgt):

        # breaking many-to-many alignments
        if ' ' in _src and ' ' in _tgt and len(_src.split()) == len(_tgt.split()):
            word_alignment['src'].extend(_src.split())
            word_alignment['tgt'].extend(_tgt.split())
        else:
            word_alignment['src'].append(_src)
            word_alignment['tgt'].append(_tgt)
        
    return word_alignment



def char_level_alignment(aligned_sents):
    """Generating char level alignment for a pair of sentences that
    are aligned at the word level

    Args:
        aligned_sents (dict): pair of word-level aligned sentences

    Returns:
        char_alignment (dict): pair of char-level aligned sentences
    """
    ...
    for i, (src_word, tgt_word) in enumerate(zip(aligned_src, aligned_tgt)):
        b_align = align_chars(src_word, tgt_word)
        basic_alignment.append(b_align)

    # clean up the basic alignment
    clean_alignment = post_process_alignment(basic_alignment, is_char_align=True)
    
    # checking the alignment
    char_alignment = {'src': [], 'tgt': []}
    ...
    for i, aligned_words in enumerate(clean_alignment):
        ...
        assert ''.join(src_word_chars) == aligned_src[i]
        assert ''.join(tgt_word_chars) == aligned_tgt[i]

        char_alignment['src'].append(src_word_chars)
        char_alignment['tgt'].append(tgt_word_chars)

    assert (len(char_alignment['src']) == len(char_alignment['tgt'])
            == len(aligned_src) == len(aligned_tgt))
