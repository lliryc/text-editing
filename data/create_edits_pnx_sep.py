...
if __name__ == '__main__':
    ...
    if args.create_edits:
        # loading the data
        data = load_data(os.path.join(args.input_data_dir, f'{output_dir}/{split}_edits.json'))
        ...
        compressed_no_pnx_data = compress_edits(no_pnx_edits_data, verify=False)

        write_json(path=os.path.join(args.output_data_dir, f'{output_dir}/{split}_edits_nopnx.json'),
                   data=compressed_no_pnx_data)
    ...
    if args.prune:
        ...
        if args.prune_cor:
            ...
            nopnx_pruned_data = prune_edits(nopnx_data, k=args.k)
            pnx_pruned_data = prune_edits(pnx_data, k=args.k)


        write_json(path=os.path.join(prune_output_dir, f'{output_dir}/{split}_edits_nopnx.json'),
                   data=nopnx_pruned_data)
