...
def prompt_template_gec_n_shot(n, sent, lang, examples_train):
    if lang == 'en':
        prompt_prelim = ('### Instruction:You are an Arabic grammatical error correction '
        ...
        prompt =  f'{prompt_prelim} {prompt_instruct} Complete the conversation between [|Human|] and [|AI|]:\n### Input: [|Human|] <input> {sent} </input> \n[|AI|]\n### Response :'

    elif lang == 'ar':
        prompt_prelim = ('### Instruction:أنت أداة لتصحيح الأخطاء النحوية والإملائية في اللغة العربية، حيث يمكنك تحديد'
        ...
        prompt = f'{prompt_prelim} {prompt_instruct} أكمل المحادثة بين [|Human|] و[|AI|] :\n### Input:[|Human|] <input> {sent} </input> \n[|AI|]\n### Response :"'
    return prompt


def prompt_template_coda_n_shot(n, sent, lang, examples_train):
    if lang == 'en':
        prompt_prelim = ('### Instruction:You are a dialectal Arabic text normalization '
        ...
        prompt_instruct = ('Please standardize the following sentence marked with the tag <input> SRC </input> into the '
                        ...
                        'Output the normalized sentence directly in Arabic script without providing any explanations. ')
        if n:
            prompt_prelim += (' We will provide you with example sentences marked with the tag <input> SRC </input>, '
        ...
        prompt_prelim = ('### Instruction:أنت أداة لتصحيح النصوص العربية العامية، حيث يمكنك تصحيح النصوص العامية إلى الإملاء القياسي للعامية العربية (CODA). '
...
def pred(arguments):
    ...
    if task == 'gec':
        prompt = prompt_template_gec_n_shot(n_shot, sent, lang, examples)
    elif task == 'coda':
        prompt = prompt_template_coda_n_shot(n_shot, sent, lang, examples)
    ...
    with open(os.path.join(output_dir, f'{sent_id}.json'), 'w') as f:
        json.dump({'input': prompt, 'output': pred}, f, ensure_ascii=False)

    return pred

def get_prompt(arguments):
    sent, sent_id, dataset_name, split, output_dir, n_shot, task, examples = arguments
    if task == 'gec':
        prompt = prompt_template_gec_n_shot(n_shot, sent, examples)
    elif task == 'coda':
        prompt = prompt_template_coda_n_shot(n_shot, sent, examples)

    return {'sent_id': sent_id, 'dataset_name': dataset_name, 'split': split, 'output_dir': output_dir, 'prompt': prompt}    
...
if __name__ == "__main__":
    ...
    parser.add_argument("--datasets", nargs='+', default=['qalb14', 'zaebuc', 'madar'],
    ...
    parser.add_argument("--lang", default='en', help='The language of the prompts')
    ...
    for dataset_name, split in DATASETS:
        ...
        preds = [pred(sent_info) for sent_info in data_]